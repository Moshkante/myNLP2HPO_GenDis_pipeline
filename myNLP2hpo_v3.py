...
stemmed_singlekw_low = set([LancasterStemmer().stem(kw).lower() for kw in singlekw_low])
stemmed_multiplekw_low = set([LancasterStemmer().stem(kw).lower() for kw in multiplekw_low])
stemmed_singlesyn_low = set([LancasterStemmer().stem(syn).lower() for syn in singlesyn_low])
stemmed_multiplesyn_low = set([LancasterStemmer().stem(syn).lower() for syn in multiplesyn_low])
stemmed_morethanonesingsyn_low = set([LancasterStemmer().stem(syn).lower() for syn in morethanonesingsyn_low])
stemmed_morethanonemultsyn_low = set([LancasterStemmer().stem(syn).lower() for syn in morethanonemultsyn_low])
...
singlekw_dict_stem = {LancasterStemmer().stem(kw).lower(): kw for kw in singlekw}
multiplekw_dict_stem = {LancasterStemmer().stem(kw).lower(): kw for kw in multiplekw}
singlesyn_dict_stem = {LancasterStemmer().stem(syn).lower(): syn for syn in singlesyn}
multiplesyn_dict_stem = {LancasterStemmer().stem(syn).lower(): syn for syn in multiplesyn}
morethanonesingsyn_dict_stem = {LancasterStemmer().stem(syn).lower(): syn for syn in morethanonesingsyn}
morethanonemultsyn_dict_stem = {LancasterStemmer().stem(syn).lower(): syn for syn in morethanonemultsyn}
...
def myNLP2hpo_v3(input_path, output_path, negation=False):
    """Main function:
    Args:
    ...
    for clin in clin_notes:
        ...
        if negation is True:
            ...
            # in case some observation is reported as negative (often the sentence ends with "negative")
            for num, tok in enumerate(tokens, start=0):
                if tok.lower() == "negative":
                    while tokens[num-1] not in punctuations:
                        tokens[num-1] = "Not_"+tokens[num-1]
                        num = num-1
                    while tokens[num+1] not in punctuations:
        ...
        for syn in multiplesyn_low:
            if LancasterStemmer().stem(syn) in HP_synonyms_list:
                continue
            else:
                if syn in note.lower():
                    HP_synonyms_list.append(syn)
        ...
        for kw in multiplekw_low:
            if LancasterStemmer().stem(kw) in HP_keywords_list:
                continue
            else:
                if kw in note.lower():
                    HP_keywords_list.append(kw)
